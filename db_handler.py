...
class Db_handler(Db_basic):
    ...
    def get_total_donations_by_donor_name(self, name):
        ...
        try:
            tb = pandas.read_sql_query(query, self.connection, params=[first_name.capitalize(), last_name.capitalize()])
            return tb.to_string()
        except NameError as e:
            print(e)
    ...
    def get_donations_by_donor_id(self, id):
        ...
        data = self.add_join_clause('donation')
        query = """
        SELECT donation_date, donation_id, amount, source,
        event_name, project_name FROM donation
        """
        query+= data['join_string']
    ...
    def check_donation_allocation(self, id):
        ...
        try:
            return pandas.read_sql_query(query, self.connection, params=[id]).to_string()
        except NameError as e:
            print(e)
    ...
    def get_donor_by_name(self, name):
        ...
        try:
            return pandas.read_sql_query(
                query, self.connection, params=[
                    first_name.capitalize(), last_name.capitalize()]).to_string()
        except NameError:
            self.cursor.execute(query, (first_name.capitalize(), last_name.capitalize()))
            return self.cursor.fetchall()
